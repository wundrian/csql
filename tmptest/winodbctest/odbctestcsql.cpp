// odbctestcsql.cpp : Defines the entry point for the console application.
//
...
{ 
	...
	printf("Allocate env handle\n");
	
    //we need ODBC3 support
    SQLSetEnvAttr(env,SQL_ATTR_ODBC_VERSION,(void*)SQL_OV_ODBC3,0);
	printf("Allocate setenv handle\n");
	
    //Allocate a Connection handle
    ret = SQLAllocHandle(SQL_HANDLE_DBC,env,&dbc);
    checkrc(ret,__LINE__);
	printf("Allocate conn handle\n");
	
	char dsn[1024];
	strcpy(dsn, "DSN=mycsql;MODE=csql;SERVER=192.168.1.3;PORT=5678;");
