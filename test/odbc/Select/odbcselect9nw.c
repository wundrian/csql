...
//SQLGetData() DSN=mycsql;MODE=csql;SERVER=127.0.0.1;PORT=5678; 
#include<stdio.h>
#include<stdlib.h>
#include<sql.h>
#include<sqlext.h>
...
int FetchTest(SQLHANDLE env, SQLHANDLE dbc, SQLHANDLE stmt)
{
    ...
    ret = SQLPrepare(stmt,(unsigned char*)"SELECT F1,F2  FROM T1 ",SQL_NTS);
    rettype = ret;
...
/*
  ...
  */  
   ...
   while(SQL_SUCCEEDED(ret = SQLFetch(stmt)))
    {
        ret=SQLGetData(stmt, 1, SQL_C_SLONG, &f1, 0,&slen );
        checkrc(ret,__LINE__);
        ret=SQLGetData(stmt, 2, SQL_C_SLONG, &f2, 0,&slen );
        checkrc(ret,__LINE__);
        count++;
        printf("F1=%d\tF2=%d\n",f1,f2);        
    }
