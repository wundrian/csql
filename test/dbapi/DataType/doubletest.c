...
int main()
{
    ...
    TableDef tabDef;
    tabDef.addField("f1", typeDouble, 0 , NULL, true);
    ...
    table->execute();
    while ((tuple = (char*) table->fetch())) {
        printf("after delete tuple present. Its  value is %g %s \n", id, name);
    }
    table->close();
    conn.commit();
