...
then
    echo "CREATE TABLE t1(f1 INT,f2 SMALLINT,f3 CHAR(30),f4 REAL,f5 REAL,f6 DATE,f7 TIME,f8 DATETIME,f9 INT,f10 BIGINT);" > create.sql   
elif [ $DSN = "oracle" ]
then
...
if [ $VALGRIND = "true" ]
then
    valgrind --tool=memcheck --leak-check=full ${REL_PATH}/selecttest1 2> $0.valgr
else
    ${REL_PATH}/selecttest1
    if [ $? -ne 0 ]
    then
       isql $DS < drop.sql >/dev/null 2>&1
