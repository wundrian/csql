#!/bin/ksh
#Testing TIMESTAMP_ADD function by adding year value
...
# select * from t1;
# select * from t1 where timestamp_add(f1 interval 5 year) ='2008-01-01 01:01:01';
# select * from t1 where timestamp_add(f1 interval 5 year)<='2008-01-01 01:01:01';
# select * from t1 where timestamp_add(f1 interval 5 year) >'2008-01-01 01:01:01';
# select * from t1 where timestamp_add(f1 interval 5 year)!='2008-01-01 01:01:01';
#
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -s ${REL_PATH}/timestamp_add_year.sql
else 
    $CSQL_INSTALL_ROOT/bin/csql -s ${REL_PATH}/timestamp_add_year.sql
    if [ $? -ne 0 ]
    then
