#!/bin/ksh
# Testing Relational Operators(=,<,>,<=,>=,!=) with varchar
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -s ${REL_PATH}/varchar_relational.sql
else 
    $CSQL_INSTALL_ROOT/bin/csql -s ${REL_PATH}/varchar_relational.sql 
    if [ $? -ne 0 ]
    then
