#!/bin/ksh
# Testing LIKE, IN Operators with varchar in Delete Statement. ( )
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -s ${REL_PATH}/varchar_like_in_del.sql
else 
    $CSQL_INSTALL_ROOT/bin/csql -s ${REL_PATH}/varchar_like_in_del.sql 
    if [ $? -ne 0 ]
    then
