...
# For inuse stmts, it should not delete underlying objects as it dumps core 
# when any method on that stmt is called after that
# Verify this by keeping open insert prepared stmt, then create index and then insert again.
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $REL_PATH/stmtTest8
else 
    $REL_PATH/stmtTest8 
    if [ $? -ne 0 ]
    then
