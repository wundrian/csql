#!/bin/ksh
# Inner Join test using explain plan(Tree index on both table).
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -s $REL_PATH/innerjoin_tree_int.sql
else
    $CSQL_INSTALL_ROOT/bin/csql -s $REL_PATH/innerjoin_tree_int.sql
    if [ $? -ne 0 ]
    then
