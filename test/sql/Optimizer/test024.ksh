#!/bin/ksh
# Self Join test using explain plan(Hash index on both table).
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -s $REL_PATH/selfjoin_hash_int.sql
else
    $CSQL_INSTALL_ROOT/bin/csql -s $REL_PATH/selfjoin_hash_int.sql
    if [ $? -ne 0 ]
    then
