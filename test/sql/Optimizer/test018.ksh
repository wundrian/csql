#!/bin/ksh
# EquiJoin and RangeJoin test using explain plan(Hash index on both table).
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -s $REL_PATH/join_hash_int.sql
else
    $CSQL_INSTALL_ROOT/bin/csql -s $REL_PATH/join_hash_int.sql
    if [ $? -ne 0 ]
    then
