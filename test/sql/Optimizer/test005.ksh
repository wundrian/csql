#!/bin/ksh
# IN, BETWEEN and LIKE operator test on Hash Index
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -s $REL_PATH/hash_inbetweenlike.sql
else
    $CSQL_INSTALL_ROOT/bin/csql -s $REL_PATH/hash_inbetweenlike.sql
    if [ $? -ne 0 ]
    then
