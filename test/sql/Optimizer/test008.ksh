#!/bin/ksh
# Create table with 4 fields all are tinyint.
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -s $REL_PATH/tiny_composite.sql
else
    $CSQL_INSTALL_ROOT/bin/csql -s $REL_PATH/tiny_composite.sql
    if [ $? -ne 0 ]
    then
