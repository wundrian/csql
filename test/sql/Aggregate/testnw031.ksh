...
# Insert 20 records with Null value in some field.
...
# Select records using Group by on multiple fields and having on count().
...
$CSQL_INSTALL_ROOT/bin/csql -o 4 -s ${REL_PATH}/alldatatype_aggregate_count.sql
if [ $? -ne 0 ]
then
   $CSQL_INSTALL_ROOT/bin/csql -s ${REL_PATH}/dropt1.sql
   exit 2;
