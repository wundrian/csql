/* select tuple with WHERE clause having param for some fields
   ...
   Insert 5-6 records
   Execute SELECT * FROM T1 WHERE f1='now' AND f2=? AND f3=?; 
   Author : Nihar Paital.
*/
...
int main()
{
    ...
    stmt->execute(rows);
    stmt->prepare("INSERT INTO T1 values('now',1,'Nihar1');");
    stmt->execute(rows);
    stmt->prepare("INSERT INTO T1 values('2008-10-24',2,'Nihar2');");
    stmt->execute(rows);
    stmt->prepare("INSERT INTO T1 values('now',3,'Nihar3');");
    stmt->execute(rows);
    stmt->prepare("INSERT INTO T1 values('2008-10-21',4,'Nihar4');");
    stmt->execute(rows);
    stmt->prepare("INSERT INTO T1 values('now',5,'Nihar5');");
    stmt->execute(rows);
    con->commit();
    stmt->free();
    //**********************************************************
    ...
    printf("SELECT * FROM T1 WHERE F1='now';\n");
    strcpy(statement,"SELECT * FROM T1 WHERE F1='now';");
    stmt->prepare(statement);
    stmt->bindField(1,&f1var);
    stmt->bindField(2,&f2var);
    stmt->bindField(3,f3var);
    ...
    printf("SELECT * FROM T1 where F1='now' and F2 >= ? and F3 <= ?;\n");
    strcpy(statement,"SELECT * FROM T1 where F1='now' and  F2 >= ? and F3 <= ?;");
    stmt->prepare(statement);
    f2var=1;
    strcpy(f3var,"Nihar3");
    stmt->setIntParam(1,f2var);
