...
FILE=${PWD}/sqlapi/Csql/DDLStmt/hashindexonalldatatypen
REL_PATH=.
if [ -s "$FILE" ]
then
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full ${REL_PATH}/hashindexonalldatatypen
else
    $REL_PATH/hashindexonalldatatypen
fi
