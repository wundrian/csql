...
// insert into statement  
    ...
    rv = stmt->prepare(statement);
    if(rv!=OK) { con->disconnect(); delete stmt; delete con; return 4; }
    rv = con->beginTrans();
    if(rv!=OK) { con->disconnect(); delete stmt; delete con; return 5; }
    ...
    rv = stmt->prepare(statement);
    if(rv!=OK) { con->disconnect(); delete stmt; delete con; return 10; }
    int id=10;
    stmt->bindField(1,&id);
    rv = con->beginTrans();
    if(rv!=OK)return 11;
