...
# Start csqlserver. Then create Composite Unique Index on two fieldse in CSQL Client.Insert 5 Records. 
# kill the currently running Server. 
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -u root -p manager -s ${REL_PATH}/create_10.sql 2> $0.valgr
else
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -u root -p manager -s ${REL_PATH}/compo_index2_10.sql 2>> $0.valgr
else
    $CSQL_INSTALL_ROOT/bin/csql -u root -p manager -s ${REL_PATH}/compo_index2_10.sql >/dev/null 2>&1
    if [ $? -ne 0 ]
     then
