...
# make Set autocommit off. insert another 5 records and Commit.
# Kill the Server. 
# Again Restart the Server by setting DURABILITY=true .
# select should Display all 10 records.
# 
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -u root -p manager -s ${REL_PATH}/ins_commit.sql 2>> $0.valgr
else
    $CSQL_INSTALL_ROOT/bin/csql -u root -p manager -s ${REL_PATH}/ins_commit.sql >/dev/null 2>&1
    if [ $? -ne 0 ]
     then
