...
#  
#    1. cache one table from each DSN.
#    2. Now use "cachetable -d <dsName>" to cache all tables    
...
sleep 5
      ...
      #### cache single table from DSNs #####
echo " "

echo "Cache t1 table from MySQL."
$CSQL_INSTALL_ROOT/bin/cachetable -t t1 -d $MYSQL_DSN
if [ $? -ne 0 ]
  then
     kill -9 $pid
      ipcrm -M 1199 -M 2277
...
echo "Cache t3 table from Postgres."
$CSQL_INSTALL_ROOT/bin/cachetable -t t3 -d $POSTGRES_DSN
if [ $? -ne 0 ]
then
    kill -9 $pid
    ipcrm -M 1199 -M 2277
...
echo "Cache t5 table from DB2."
$CSQL_INSTALL_ROOT/bin/cachetable -t t5 -d $DB2_DSN
if [ $? -ne 0 ]
then
    kill -9 $pid
    ipcrm -M 1199 -M 2277
...
echo "Cache t7 table from Oracle."
$CSQL_INSTALL_ROOT/bin/cachetable -t t7 -d $ORACLE_DSN
if [ $? -ne 0 ]
then
    kill -9 $pid
    ipcrm -M 1199 -M 2277
