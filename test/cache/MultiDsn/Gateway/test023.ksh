...
#   DELTE records from  t1,t2 tables in MySQL. 
#   DELTE records from  tt1,tt2 tables in Postgres.
...
echo "Table 'tt1' is cached from $POSTGRES_DSN."
        ...
        ####### UPDATE cache table as well as non-cache table ######

echo ""
echo "DELETE records from tables using Gateway(csql -g)"
echo "DELETE records from t1 and t2 table."
$CSQL_INSTALL_ROOT/bin/csql -g -s $REL_PATH/mysqldelete.sql
if [ $? -ne 0 ]
then
   kill -9 $pid
   ipcrm -M 1199 -M 2277
...
echo "DELETE records from tt1 and tt2 table."
$CSQL_INSTALL_ROOT/bin/csql -g -s $REL_PATH/psqldelete.sql
if [ $? -ne 0 ]
then
   kill -9 $pid
   ipcrm -M 1199 -M 2277
