...
#   UPDATE t1,t2 tables in MySQL. 
#   UPDTAE tt1,tt2 tables in Postgres.
...
echo ""
echo "UPDATE tables using Gateway(csql -g)"
echo "update t1 and t2 table."
$CSQL_INSTALL_ROOT/bin/csql -g -s $REL_PATH/mysqlupdate.sql
if [ $? -ne 0 ]
then
   kill -9 $pid
   ipcrm -M 1199 -M 2277
...
echo "update tt1 and tt2 table."
$CSQL_INSTALL_ROOT/bin/csql -g -s $REL_PATH/psqlupdate.sql
if [ $? -ne 0 ]
then
   kill -9 $pid
   ipcrm -M 1199 -M 2277
