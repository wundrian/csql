...
#  3. Create table t1 in CSQL with all supported data types in it. and insert 5 records in it using csql tool with -g option.
#  Note that in this test case the table is not cached in csql and is present in csql
#  insert 5 more records using csql tool with -g option.
#  select records with WHERE clause containing parameters for all the fields with different data types.
#  update all fields with WHERE clause on primary key using csql tool with -g option.
#  delete records with WHERE clause on all fields using csql tool with -g option
...
if [ $DSN = "oracle" ]
then
   $CSQL_INSTALL_ROOT/bin/csql -g -s $REL_PATH/orainput_t1.oracle  >/dev/null 2>&1
else
...
then
   ...
   $CSQL_INSTALL_ROOT/bin/csql -g -s $REL_PATH/oraselect.sql
  ...
  if [ $DSN = "db2" ]
  then
      $CSQL_INSTALL_ROOT/bin/csql -g -s $REL_PATH/csqldb2inputtest3a.sql >/dev/null 2>&1 
      $CSQL_INSTALL_ROOT/bin/csql -g -s $REL_PATH/select.sql 
      $CSQL_INSTALL_ROOT/bin/csql -g -s $REL_PATH/db2update.sql >/dev/null 2>&1   
  else
