...
$CSQL_INSTALL_ROOT/bin/csql -H localhost -P 6008 -g -s $REL_PATH/selectstar.sql
if [ $? -ne 0 ]
then
...
if [ $DSN = "oracle" ]
then
  $CSQL_INSTALL_ROOT/bin/csql -H localhost -P 6008 -g -s $REL_PATH/complex.oracle
elif [ $DSN = "sybase" -o $DSN = "db2" ]
then
  $CSQL_INSTALL_ROOT/bin/csql -H localhost -P 6008 -g -s $REL_PATH/complex.sybase
else
  $CSQL_INSTALL_ROOT/bin/csql -H localhost -P 6008 -g -s $REL_PATH/complex.sql
fi
