#!/bin/ksh
# Test Case
#cacheverify test after all Non-select dml (primary key on bigint field) (cache/CacheVerify/test010.ksh)
#Create table t1 in mysql with primary key on bigint field. Insert 5 rows.
...
if [ $DSN = "myodbc3" ]
then
   isql $DS < ${REL_PATH}/mysqlinput12.sql >/dev/null 2>&1
elif [ $DSN = "oracle" ]
then
   isql $DS < ${REL_PATH}/oracleinput12.sql >/dev/null 2>&1
elif [ $DSN = "sybase" ]
then
   isql $DS < ${REL_PATH}/sybaseinput12.sql >/dev/null 2>&1
elif [ $DSN = "db2" ]
then
   isql $DS < ${REL_PATH}/db2input12.sql >/dev/null 2>&1
else
   isql $DS < ${REL_PATH}/psqlinput12.sql >/dev/null 2>&1
fi
if [ $? -ne 0 ]
then
    rm -f /tmp/csql/csqltable.conf
