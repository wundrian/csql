...
# Create table t1 with field f1 int ,f2 char,f3 smallint , f4 int primary key(f1).Make bidirectional flag and cache flag true. Cache the table with cachetable -t t1 -f f1,f2,f3 -p f4 it should fail
...
sleep 5
echo "server  started"
$CSQL_INSTALL_ROOT/bin/cachetable -t t1 -f f1,f2,f3 -p f4
if [ $? -eq 0 ]
then
