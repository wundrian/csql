...
# Create table t1 with field f1 int unique ,f2 char,f3 smallint , f4 bigint . Make bidirectional flag and cache flag true. Cache the table with cachetable -t t1 -f f1,f2,f3 -p f1 it should pass 
...
echo "create table t1(f1 int unique,f2 char(12),f3 smallint , f4 int);">$REL_PATH/t1.sql
if [ $DSN = "db2" ]
then
    echo "create table t1(f1 int ,f2 char(12),f3 smallint , f4 int);">$REL_PATH/t1.sql
fi
...
then
    $CSQL_INSTALL_ROOT/bin/cachetable -t t1 -p f1 -F
    if [ $? -ne 0 ]
    then
