...
#   Create table t1 with field f1 int ,f2 char,f3 smallint , f4 bigint primary key(f1) . Make bidirectional flag and cache flag true. Cache the table with cachetable -t t1 -f f2,f3 it should fail
...
echo "create table t1(f1 int ,f2 char(12),f3 smallint , f4 bigint,primary key(f1));">$REL_PATH/t1.sql
...
sleep 5
echo "server  started"
$CSQL_INSTALL_ROOT/bin/cachetable -t t1 -f f2,f3,f4
if [ $? -eq 0 ]
then
    echo "Table is cached test case failed"
