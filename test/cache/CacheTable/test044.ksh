...
# Bi-directional cache  test, Describe with -S option.
...
echo "create table t1(f1 int unique,f2 char(12),f3 int,f4 int,primary key(f2));">$REL_PATH/t1.sql
if [ $DSN = "db2" ]
then
    echo "create table t1(f1 int not null unique,f2 char(12) not null ,f3 int,f4 int,primary key(f2));">$REL_PATH/t1.sql
fi
...
sleep 5
echo "server  started"
echo "===========================  cachetable -t t1 -c \"f1>12\" -f \"f1,f2\" -p f1  =======================================";
if [ "$DSN" = "oracle" -o "$DSN" = "sybase" -o "$DSN" = "db2"  ]
then
{
    $CSQL_INSTALL_ROOT/bin/cachetable -t t1 -c "f1>12" -f "f1,f2" -p f1 -F >/dev/null 2>&1
    if [ $? -ne 0 ]
    then
...
{
    $CSQL_INSTALL_ROOT/bin/cachetable -t t1 -c "f1>12" -f "f1,f2" -p f1  >/dev/null 2>&1
    if [ $? -ne 0 ]
    then
       echo "Unable to cache"
       isql $DS <${REL_PATH}/dt1.sql  >/dev/null 2>&1
...
$CSQL_INSTALL_ROOT/bin/cachetable -S
if [ $? -ne 0 ]
then
   echo "Unable to cache"
   isql $DS <${REL_PATH}/dt1.sql  >/dev/null 2>&1
