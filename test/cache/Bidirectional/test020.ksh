...
# Create table t1 with field f1 int unique ,f2 char,f3 smallint , f4 bigint . Make bidirectional flag and cache flag true. run trigger ,create log table in target db.Cache the table by -c f1>10 -f f1,f2,f3 -p f1 . 
# Insert record with 4 field .and select from t1 it should shows records having fa gretter than 10 in 3 field.
# Author:= Bijaya
...
echo "create table t1(f1 int ,f2 char(12),f3 int, f4 int);">$REL_PATH/t1.sql
