...
# Create table t1 with field f1 int unique, f2 char,f3 smallint , f4 bigint ,primary key(f2,f3) . Make bidirectional flag and cache flag true. run trigger ,create log table in target db.Cache the table by -f f1,f2,f3 -p f1 . insert record with 4 field .and select from t1 it should three field values.
# Author:= Bijaya
...
if [ $DSN = "db2" ]
then 
echo "create table t1(f1 int not null,f2 char(12) not null,f3 int, f4 int,primary key(f2));">$REL_PATH/t1.sql
fi
...
then
{
$CSQL_INSTALL_ROOT/bin/cachetable -t t1 -c "f1>10" -f f1,f3,f2 -p f1 -F
if [ $? -ne 0 ]
then
...
{
$CSQL_INSTALL_ROOT/bin/cachetable -t t1 -c "f1>10" -f f1,f3,f2 -p f1
if [ $? -ne 0 ]
then
    echo "Unable to cache"
    isql $DS <${REL_PATH}/dt1.sql  >/dev/null 2>&1
...
then
    ...
    isql $DS <${REL_PATH}/mysqldeletelogtable.sql >/dev/null 2>&1
    kill `ps -e | grep csqlcacheserver | awk -F" " '{ print $1 }'`
    kill -9 $pid
    ipcrm -M 4000 -M 4500
    exit 4
fi
