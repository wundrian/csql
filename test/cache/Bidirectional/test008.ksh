...
# insert operation in one cache node database
...
#echo $DSQL_CONFIG_FILE /tmp/csql.conf
export CSQL_CONFIG_FILE=/tmp/csql.conf
...
isql $DS < $TRI_FILE1 >/dev/null 2>&1 
isql $DS < $TRI_FILE2  >/dev/null 2>&1
...
$CSQL_INSTALL_ROOT/bin/csql -g < ${REL_PATH}/insert.sql >/dev/null 2>&1
sleep 20 
echo "cache node 1"
$CSQL_INSTALL_ROOT/bin/csql  -s $REL_PATH/select.sql
