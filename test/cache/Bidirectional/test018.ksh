...
#Create table t1 with field f1 int ,f2 char,f3 smallint , f4 bigint, primarykey(f1) . Make bidirectional flag and cache flag true. run trigger ,create log table in target db.Cache the table by -c f1>10 . 
# Insert record in target db with f1 values as f1>10 and f1<10 and f1 =10 and select from t1 it should shows records having f1 gretter than 10.
# Author:= Bijaya
...
$CSQL_INSTALL_ROOT/bin/cachetable -t t1 -c "f1>10"
if [ $? -ne 0 ]
then
