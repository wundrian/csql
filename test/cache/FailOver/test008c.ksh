...
#  8c. After connect, prepare, execute, CSQL goes down.fetch() should return records from MySQL for cached and MySQL table.
...
if [ $DSN = "oracle" ]
then
   ${REL_PATH}/updateexeccsqldownora  &
else
   ${REL_PATH}/updateexeccsqldown &
fi
bgproc=$!
kill -9 $pid 
ipcrm -M 1199 -M 2277
