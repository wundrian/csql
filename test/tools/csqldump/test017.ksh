...
#
input=${PWD}/tools/csqldump/foreign4.sql
REL_PATH=.
if [ -s "$input" ]
then
...
if [ "$VALGRIND" = "true" ]
then
    valgrind --tool=memcheck --leak-check=full $CSQL_INSTALL_ROOT/bin/csql -u root -p manager -s $REL_PATH/foreign4.sql 2> $0.valgr
else
    $CSQL_INSTALL_ROOT/bin/csql -u root -p manager -s $REL_PATH/foreign4.sql >/dev/null 2>&1
    if [ $? -ne 0 ] 
    then 
