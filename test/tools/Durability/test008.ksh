...
# Kill the server
# Restart csqlserver again. set Autocommit Off mode.insert another 5 record.and  Rollback the transaction.
# Kill the server
# Again Restart The server
# Select Should fetch only 1st 5 records
# 
...
$CSQL_INSTALL_ROOT/bin/csql -u root -p manager -s ${REL_PATH}/insert_b.sql
if [ $? -ne 0 ]
 then
   $CSQL_INSTALL_ROOT/bin/csql -u root -p manager -s ${REL_PATH}/drop_t2.sql
   rm -f ${REL_PATH}/select_t2.sql
